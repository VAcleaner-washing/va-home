## 13.8.17 — Authenticated checkout email prefill
- Якщо покупець увійшов у кабінет, email автоматично підтягується у форму оформлення.
- Значення береться з локальної Supabase-сесії одразу та додатково перевіряється через Auth API без блокування сторінки.
- Підтягнутий email залишається доступним для редагування.
- Після ручного введення сайт більше не перезаписує адресу асинхронною відповіддю сесії.
- Email зберігається у поточній чернетці checkout, як і решта контактних даних.
- Під полем показується ненав’язлива позначка, що адресу взято з кабінету.
- Кеші клієнтського й окремого Admin PWA синхронізовано до 13.8.17.
