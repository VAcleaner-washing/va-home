...
function discoveryCreditBlock(credit:any){
  if(!credit)return "";
  ...
  const fullSet = Number(credit.amount || 0) >= 450;
  const title = fullSet ? `До ${money(credit.amount)} на повнорозмірні аромати` : `${money(credit.amount)} на повнорозмірний аромат`;
  const terms = fullSet
    ? "250 грн діють на один аромат або всі 450 грн — на замовлення від двох повнорозмірних ароматів."
    : "150 грн застосовуються до одного повнорозмірного аромату.";
  return `<div style="margin:28px 0;padding:26px;border:1px solid #C8A27C;background:#FBF9F6;border-radius:3px;"><div style="font-size:10px;letter-spacing:1.6px;color:#9A744D;text-transform:uppercase;margin-bottom:10px;">Discovery Credit</div><h2 style="margin:0 0 10px;color:#171717;font:normal 21px/1.25 'Georgia',serif;">${title}</h2><p style="margin:0 0 18px;color:#525252;font-size:14px;line-height:1.6;">${terms} Ваш персональний код створено автоматично після Discovery Set.</p><div style="padding:15px 16px;background:#EDE7DF;text-align:center;font-family:monospace;font-size:18px;letter-spacing:1.5px;color:#171717;">${esc(promo.code)}</div><p style="margin:12px 0 0;color:#8B8178;font-size:11px;line-height:1.5;">Код діє до ${esc(expires)}, прив’язаний до цього email і застосовується один раз.</p><p style="margin:18px 0 0;"><a href="https://vahome.com.ua/catalog.html" style="display:inline-block;padding:12px 20px;background:#171717;color:#fff;text-decoration:none;font-size:13px;">Обрати ${fullSet ? "аромати" : "аромат"}</a></p></div>`;
}
...
const creditBlock=o.status==="completed"?discoveryCreditBlock(discoveryCredit):"";
...
await resend(resendKey,{from:FROM_EMAIL,to:[o.customer_email],reply_to:SHOP_EMAIL,subject:`${status} — ${o.client_order_id}`,html});if(o.status==="completed"){const nowIso=new Date().toISOString();const {error:markError}=await admin.from("orders").update({review_invitation_sent_at:nowIso}).eq("id",o.id).is("review_invitation_sent_at",null);if(markError)throw markError;if(discoveryCredit?.id)await admin.from("discovery_credits").update({notified_at:nowIso,updated_at:nowIso}).eq("id",discoveryCredit.id);}return json(req,{ok:true,review_invitation:o.status==="completed",discovery_credit:Boolean(discoveryCredit)});}catch(e){console.error(e);return json(req,{error:e instanceof Error?e.message:"Unknown error"},500)}});