(function () {
  "use strict";

  const STORAGE_KEY = "vahome_last_order";
  const DEFAULT_PAYMENT_RECIPIENT = "\u0424\u041E\u041F \u041D\u0435\u0432\u0456\u0434\u043E\u043C\u0430 \u0410\u043D\u043D\u0430 \u0421\u0435\u0440\u0433\u0456\u0457\u0432\u043D\u0430";

  function formatUAH(value) { return `${Number(value || 0).toLocaleString("uk-UA")}\u00A0грн`; }
  function getOrder() { try { const raw = sessionStorage.getItem(STORAGE_KEY); return raw ? JSON.parse(raw) : null; } catch (_) { return null; } }
  function saveOrder(order) { try { sessionStorage.setItem(STORAGE_KEY, JSON.stringify(order)); } catch (_) {} }
  function copyText(text) {
    if (navigator.clipboard?.writeText) return navigator.clipboard.writeText(text);
    const area = document.createElement("textarea"); area.value = text; area.style.position = "fixed"; area.style.opacity = "0";
    document.body.appendChild(area); area.select(); document.execCommand("copy"); area.remove(); return Promise.resolve();
  }
  function setText(id, value) { const node = document.getElementById(id); if (node) node.textContent = value; }
  function normalizePaymentRecipient(value) {
    const recipient = String(value || "").trim().normalize("NFC");
    return recipient && !recipient.includes("\uFFFD") ? recipient : DEFAULT_PAYMENT_RECIPIENT;
  }
  function queryPaymentIdentity(order) {
    const params = new URLSearchParams(location.search);
    return {
      orderNumber: params.get("order") || order?.orderNumber || order?.client_order_id || "",
      token: params.get("token") || order?.paymentReturnToken || "",
      isReturn: params.get("payment") === "return" || params.get("payment") === "pending"
    };
  }
  function cleanPaymentQuery() {
    const url = new URL(location.href);
    ["payment", "order", "token"].forEach((key) => url.searchParams.delete(key));
    history.replaceState({}, "", `${url.pathname}${url.search}${url.hash}`);
  }

  function renderEmptyState() {
    setText("orderNumber", "Замовлення збережено");
    document.getElementById("orderItems").innerHTML = '<p class="ty-empty">Деталі замовлення більше не зберігаються у цьому браузері. Актуальну інформацію можна знайти в листі-підтвердженні або кабінеті.</p>';
    setText("orderTotal", "—");
    ["paymentPanel", "codPanel", "cardPaymentPanel"].forEach((id) => { const node = document.getElementById(id); if (node) node.hidden = true; });
    document.querySelector(".ty-primary-grid")?.classList.add("is-single");
    setText("orderSuccessLead", "Замовлення вже передано магазину. Перевірте лист-підтвердження або відкрийте особистий кабінет.");
  }

  function renderOrder(order) {
    setText("orderNumber", order.orderNumber || order.client_order_id || "—");
    const items = Array.isArray(order.items) ? order.items : [];
    const discountLine = Number(order.discount || 0) > 0
      ? `<div class="ty-order-line ty-order-line--discount"><span>Промокод ${String(order.promoCode || "").toUpperCase()}</span><span>−${formatUAH(order.discount)}</span></div>` : "";
    document.getElementById("orderItems").innerHTML = items.length
      ? items.map((item) => {
          const selections = Array.isArray(item.selections) && item.selections.length ? `<small>Обрано: ${item.selections.join(" · ")}</small>` : "";
          return `<div class="ty-order-line"><span>${item.name || "Товар"} × ${item.quantity || 1}${selections}</span><span>${formatUAH(item.line_total)}</span></div>`;
        }).join("") + discountLine
      : '<p class="ty-empty">Склад замовлення буде доступний у листі-підтвердженні.</p>';
    setText("orderTotal", formatUAH(order.total));
    if (order.emailStatus === "failed") setText("orderSuccessLead", "Замовлення успішно збережено. Лист може надійти із затримкою — збережіть номер замовлення.");
  }

  function cardMessage(status, reason) {
    if (status === "paid") return { title: "Оплату підтверджено", message: "Дякуємо. Платіж успішний, замовлення вже передано на комплектацію.", retry: false };
    if (status === "refunded") return { title: "Кошти повернено", message: "Повернення зафіксовано. За деталями можна звернутися до менеджера VA HOME.", retry: false };
    if (status === "failed") return { title: "Оплату не завершено", message: reason || "Банк не підтвердив платіж. Спробуйте ще раз або оберіть інший спосіб оплати.", retry: true };
    if (status === "expired") return { title: "Посилання втратило чинність", message: "Строк дії рахунку завершився. Створіть нове платіжне посилання.", retry: true };
    return { title: "Очікуємо підтвердження", message: "Платіж ще обробляється. Зазвичай це займає кілька секунд.", retry: false };
  }

  async function syncCardPayment(order) {
    const panel = document.getElementById("cardPaymentPanel");
    const retry = document.getElementById("retryCardPaymentBtn");
    const identity = queryPaymentIdentity(order);
    if (!panel || !identity.orderNumber || !identity.token || !window.VAHomeSupabase?.cardPayment) {
      setText("cardPaymentTitle", "Статус оплати в кабінеті");
      setText("cardPaymentMessage", "Платіжне посилання збережено в замовленні. Оновлений статус також з’явиться у вашому кабінеті.");
      if (retry) retry.hidden = true;
      return;
    }
    try {
      const result = await window.VAHomeSupabase.cardPayment({ action: "status", order_number: identity.orderNumber, token: identity.token });
      const view = cardMessage(result.payment_status, result.failure_reason);
      setText("cardPaymentTitle", view.title);
      setText("cardPaymentMessage", view.message);
      panel.classList.toggle("is-paid", result.payment_status === "paid");
      panel.classList.toggle("is-failed", ["failed", "expired"].includes(result.payment_status));
      if (retry) retry.hidden = !view.retry;
      order.paymentStatus = result.payment_status;
      order.paymentReturnToken = identity.token;
      saveOrder(order);
      if (result.payment_status === "paid") {
        setText("tyTitle", "Оплату\nпідтверджено.");
        setText("orderSuccessLead", "Замовлення оплачено. Ми вже готуємо його до відправлення.");
        setText("paymentNote", "Після комплектації надішлемо номер ТТН.");
      }
      cleanPaymentQuery();
    } catch (_) {
      setText("cardPaymentTitle", "Замовлення збережено");
      setText("cardPaymentMessage", "Не вдалося миттєво перевірити платіж. Статус оновиться автоматично після відповіді банку.");
      if (retry) retry.hidden = false;
    }
    if (retry) retry.onclick = async function () {
      retry.disabled = true; setText("cardPaymentStatus", "Створюємо нове платіжне посилання…");
      try {
        const result = await window.VAHomeSupabase.cardPayment({ action: "retry", order_number: identity.orderNumber, token: identity.token });
        if (result.return_token) { order.paymentReturnToken = result.return_token; saveOrder(order); }
        if (!result.payment_url) throw new Error("NO_PAYMENT_URL");
        location.assign(result.payment_url);
      } catch (_) {
        retry.disabled = false; setText("cardPaymentStatus", "Не вдалося відкрити оплату. Спробуйте ще раз трохи пізніше.");
      }
    };
  }

  document.addEventListener("DOMContentLoaded", function () {
    const order = getOrder();
    const grid = document.querySelector(".ty-primary-grid");
    const paymentPanel = document.getElementById("paymentPanel");
    const codPanel = document.getElementById("codPanel");
    const cardPanel = document.getElementById("cardPaymentPanel");
    const payment = order && (order.paymentDetails || order.payment_details);
    if (order) renderOrder(order); else renderEmptyState();

    document.getElementById("copyOrderNumberBtn")?.addEventListener("click", function () {
      const value = document.getElementById("orderNumber").textContent.trim();
      copyText(value).then(() => setText("copyOrderStatus", "Номер скопійовано")).catch(() => setText("copyOrderStatus", "Не вдалося скопіювати"));
    });

    const isCod = Boolean(order && order.paymentMethod === "cash_on_delivery");
    const isCard = Boolean(order && order.paymentMethod === "card_online");
    if (order) {
      paymentPanel.hidden = isCod || isCard;
      codPanel.hidden = !isCod;
      cardPanel.hidden = !isCard;
    }
    if (isCod) {
      setText("orderSuccessLead", "Ми отримали замовлення з оплатою при отриманні. Збережіть його номер.");
      setText("paymentNote", "Відправимо замовлення Новою поштою протягом 1–2 робочих днів. Оплата — під час отримання.");
    }
    if (isCard) {
      setText("tyTitle", "Замовлення\nзбережено.");
      setText("orderSuccessLead", "Перевіряємо карткову оплату та одразу оновимо статус замовлення.");
      setText("paymentNote", "Після підтвердження банком замовлення автоматично перейде на комплектацію.");
      syncCardPayment(order);
    }
    if (order && !isCod && !isCard) {
      const number = order.orderNumber || order.client_order_id || "номер замовлення";
      const purpose = `Оплата замовлення ${number}`;
      const recipient = normalizePaymentRecipient(payment && payment.recipient);
      const iban = payment && payment.iban;
      const details = document.getElementById("paymentDetails");
      if (recipient && iban) {
        setText("paymentTitle", "Реквізити"); setText("paymentIntro", "Скопіюйте дані одним натисканням. Призначення платежу вже сформовано.");
        details.hidden = false; setText("paymentRecipient", recipient); setText("paymentIban", iban); setText("paymentPurpose", purpose);
        document.getElementById("copyPaymentBtn")?.addEventListener("click", function () {
          copyText(`Отримувач: ${recipient}\nIBAN: ${iban}\nПризначення платежу: ${purpose}`)
            .then(() => setText("copyStatus", "Реквізити скопійовано")).catch(() => setText("copyStatus", "Не вдалося скопіювати"));
        });
      } else if (details) details.hidden = true;
    }
    if (!order && grid) grid.classList.add("is-single");
  });
})();
