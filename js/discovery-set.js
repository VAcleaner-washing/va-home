(function(){
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    ...
    $("#openDiscoveryPicker").addEventListener("click",()=>$("#discoveryPicker").showModal());
    ...
    $("#confirmDiscoverySelection").addEventListener("click",()=>{if(selected.length!==MAX)return;$("#discoveryPicker").close();$("#addDiscovery6").focus()});
    $("#addDiscovery6").addEventListener("click",()=>{if(selected.length!==MAX){$("#discoveryPicker").showModal();return}window.Cart.add("discovery-6",1,{selections:[...selected]});window.Cart.refreshCountBadge?.();window.VAHome?.showToast("Discovery Set із 6 ароматів додано в кошик")});
    render();
  });
})();
