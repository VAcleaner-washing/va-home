...
const imageCandidates=p=>{const gallery=Array.isArray(p?.images?.gallery)?p.images.gallery:[];const macro=gallery.find(item=>item?.type==='macro'&&item.src)?.src;return macro?[macro]:[]};
