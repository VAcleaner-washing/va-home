...
let orders=[],reviews=[],promos=[],releases=[],marketingPreferences=[],repeatCampaigns=[],discoveryCredits=[],paymentAttempts=[],paymentSettings=[],activeOrder=null,activePromo=null,activeRelease=null;
...
async function loadAll(){
  ...
  try{
    ...
    marketingPreferences=dataOrEmpty(optional[0]);repeatCampaigns=dataOrEmpty(optional[1]);discoveryCredits=dataOrEmpty(optional[2]);paymentAttempts=dataOrEmpty(optional[3]);paymentSettings=dataOrEmpty(optional[4]);
...
async function deletePromo(){if(!activePromo||!confirm(`Видалити промокод ${activePromo.code}?`))return;const {error}=await sb.from("promo_codes").delete().eq("id",activePromo.id);if(error)return toast("Помилка: "+error.message);promos=promos.filter(p=>p.id!==activePromo.id);$("#promoDialog").close();renderPromos();renderAdmin2();toast("Промокод видалено");}
...
async function deleteRelease(){if(!activeRelease||!confirm(`Видалити реліз ${activeRelease.title}?`))return;const {error}=await sb.from("private_releases").delete().eq("id",activeRelease.id);if(error)return toast("Помилка: "+error.message);releases=releases.filter(r=>r.id!==activeRelease.id);$("#releaseDialog").close();renderReleases();renderAdmin2();toast("Реліз видалено");}
...
function paidRevenueOrders(){return orders.filter(order=>!["cancelled","new","awaiting_payment"].includes(effectiveOrderStatus(order))||String(order.payment_status)==="paid");}
...
function activateAdmin2View(name,{updateHash=true}={}){const view=admin2Views.includes(name)?name:"overview";activeAdmin2View=view;document.querySelectorAll(".admin2-view").forEach(panel=>panel.hidden=panel.id!==`${view}Tab`);if(view==="marketing"){const releasesPanel=$("#releasesTab");if(releasesPanel)releasesPanel.hidden=false;}document.querySelectorAll("[data-admin-view]").forEach(button=>button.classList.toggle("is-active",button.dataset.adminView===view));const main=$("#admin2Main");if(main){main.scrollTop=0;main.scrollLeft=0;}if(updateHash&&location.hash!==`#${view}`)history.replaceState(null,"",`#${view}`);const more=$("#adminMoreMenu");if(more)more.hidden=true;}
function renderOverview(){const host=$("#overviewKpis");if(!host)return;const now=new Date(),todayPaid=paidRevenueOrders().filter(order=>sameDay(order.paid_at||order.created_at,now));const last30=paidRevenueOrders().filter(order=>new Date(order.paid_at||order.created_at)>=daysAgo(29));const completed=completedOrders();const aov=completed.length?sumAmount(completed)/completed.length:0;const customers=customerRows();host.innerHTML=[
  ["Виручка сьогодні",money(sumAmount(todayPaid)),`${todayPaid.length} оплачених`],
  ["30 днів",money(sumAmount(last30)),`${last30.length} замовлень`],
  ["Середній чек",money(aov),"доставлені замовлення"],
  ["Repeat-rate",`${Math.round(repeatRate())}%`,`${customers.filter(c=>c.completed>1).length} повторних клієнтів`],
  ["Нові замовлення",String(orders.filter(o=>effectiveOrderStatus(o)==="new").length),"потребують опрацювання"],
  ["Відгуки",String(reviews.filter(r=>r.status==="pending").length),"очікують модерації"]
...
function buildAttentionItems(){const out=[];orders.filter(needsAttention).forEach(order=>{const guidance=orderGuidance(order);out.push({kind:"order",id:order.id,title:`${order.client_order_id} · ${guidance.title}`,detail:`${order.customer_name} · ${orderItemSummary(order)} · ${money(order.total_amount)}`,action:"Відкрити",tone:guidance.tone,at:order.created_at});});reviews.filter(r=>r.status==="pending").forEach(r=>out.push({kind:"review",id:r.id,title:`Новий відгук · ${r.product_slug}`,detail:`${r.customer_name} · ${Number(r.rating||0)}/5`,action:"Модерувати",tone:"warning",at:r.created_at}));repeatCampaigns.filter(r=>r.status==="failed").forEach(r=>out.push({kind:"marketing",id:r.id,title:"Не відправилась repeat-кампанія",detail:r.customer_email||"Перевірте журнал кампанії",action:"Маркетинг",tone:"danger",at:r.updated_at||r.created_at}));return out.sort((a,b)=>new Date(b.at)-new Date(a.at));}
...
function renderAttention(){const list=buildAttentionItems(),host=$("#attentionList"),empty=$("#attentionEmpty"),summary=$("#attentionSummary");if(!host||!summary)return;const payment=orders.filter(hasPaymentIssue).length,pendingReviews=reviews.filter(r=>r.status==="pending").length,oldNew=orders.filter(o=>effectiveOrderStatus(o)==="new"&&orderAgeMinutes(o)>60).length,failedCampaigns=repeatCampaigns.filter(r=>r.status==="failed").length;summary.innerHTML=[["Усього",list.length,"конкретних дій"],["Оплати",payment,"проблемних платежів"],["Відгуки",pendingReviews,"на модерації"],["Маркетинг",failedCampaigns,"невдалих кампаній"]].map(([l,v,s])=>`<article class="admin2-kpi"><span>${l}</span><strong>${v}</strong><small>${s}</small></article>`).join("");if(empty)empty.hidden=!!list.length;host.innerHTML=list.map(item=>`<article class="admin2-attention-card ${item.tone==="danger"?"is-danger":""}" data-attention-kind="${esc(item.kind)}" data-attention-id="${esc(item.id||"")}"><div class="admin2-attention-icon">${item.kind==="order"?"→":item.kind==="review"?"★":"!"}</div><div><strong>${esc(item.title)}</strong><p>${esc(item.detail)}</p><time>${shortDate(item.at)}</time></div><span class="admin2-attention-action">${esc(item.action)}</span></article>`).join("");bindAttentionClicks(host);}
...
stats.innerHTML=[["Підписані",subs,"email-згода"],["Discovery Credit",activeCredits,`${usedCredits} використано`],["Repeat черга",pending,"очікують відправки"],["Repeat sent",sent,"успішно надіслано"]].map(([l,v,s])=>`<article class="admin2-kpi"><span>${l}</span><strong>${v}</strong><small>${s}</small></article>`).join("");
const repeatHost=$("#repeatCampaignList");
if(repeatHost)repeatHost.innerHTML=repeatCampaigns.slice(0,8).map(r=>`<div class="admin2-campaign-row"><header><strong>${esc(r.customer_email||"Клієнт")}</strong><span class="admin2-state ${r.status==="failed"?"failed":r.status==="sent"?"paid":"pending"}">${esc(r.status||"pending")}</span></header><p>${r.scheduled_for?`Заплановано ${shortDate(r.scheduled_for)}`:""}${r.sent_at?` · Надіслано ${shortDate(r.sent_at)}`:""}</p></div>`).join("")||'<div class="admin2-search-empty">Repeat-кампаній ще немає.</div>';
const creditHost=$("#discoveryCreditList");
if(creditHost)creditHost.innerHTML=discoveryCredits.slice(0,8).map(r=>`<div class="admin2-credit-row"><header><strong>${esc(r.customer_email||"Клієнт")}</strong><span class="admin2-state ${r.status==="used"?"paid":r.status==="active"?"pending":"failed"}">${esc(r.status||"—")}</span></header><p>${money(r.amount)} · до ${r.expires_at?shortDate(r.expires_at):"—"}</p></div>`).join("")||'<div class="admin2-search-empty">Discovery Credit ще не видавались.</div>';
...
stats.innerHTML=[["Карткою",card.length,`${card.filter(o=>o.payment_status==="paid").length} успішних`],["На рахунок",bank.length,"ручне підтвердження"],["Післяплата",cod.length,"оплата при отриманні"],["Проблемні",issues.length,"failed / expired"]].map(([l,v,s])=>`<article class="admin2-kpi"><span>${l}</span><strong>${v}</strong><small class="${l==="Проблемні"&&v?"bad":""}">${s}</small></article>`).join("");
...
function renderAnalytics2(){const kpis=$("#analyticsKpis");if(!kpis)return;const comp=completedOrders(),customers=customerRows(),last30=paidRevenueOrders().filter(o=>new Date(o.paid_at||o.created_at)>=daysAgo(29));kpis.innerHTML=[["Виручка 30 днів",money(sumAmount(last30)),`${last30.length} замовлень`],["AOV",money(comp.length?sumAmount(comp)/comp.length:0),"доставлені"],["Repeat-rate",`${Math.round(repeatRate())}%`,`${customers.filter(c=>c.completed>1).length} клієнтів`],["Унікальні клієнти",customers.length,"за історію"]].map(([l,v,s])=>`<article class="admin2-kpi"><span>${l}</span><strong>${v}</strong><small>${s}</small></article>`).join("");const products=$("#analyticsTopProducts"),rows=productSales().slice(0,8),max=Math.max(1,...rows.map(r=>r.qty));if(products)products.innerHTML=rows.map(row=>`<div class="admin2-product-bar"><header><strong>${esc(row.product.name)}</strong><span>${row.qty} шт · ${money(row.revenue)}</span></header><i style="--value:${Math.round(row.qty/max*100)}%"></i></div>`).join("");renderOverviewPaymentMixInto($("#analyticsPayments"));}
...
function renderGlobalSearch(){const input=$("#adminGlobalSearch"),host=$("#adminSearchResults");if(!input||!host)return;const rows=globalSearchEntries(input.value);if(input.value.trim().length<2){host.hidden=true;host.innerHTML="";return;}host.hidden=false;host.innerHTML=rows.map((r,i)=>`<button type="button" class="admin2-search-result" data-global-index="${i}"><small>${esc(r.type)}</small><div><strong>${esc(r.title)}</strong><span>${esc(r.sub)}</span></div><span>→</span></button>`).join("")||'<div class="admin2-search-empty">Нічого не знайдено.</div>';host.querySelectorAll("[data-global-index]").forEach(btn=>btn.onclick=()=>{const row=rows[Number(btn.dataset.globalIndex)];host.hidden=true;input.value="";row?.action();});}
function manualProductOptions(){return fullSizeProducts().map(p=>`<option value="${esc(p.id)}">${esc(p.name)} · ${money(productPrice(p))}</option>`).join("");}
...
function resetManualOrder(){manualItems=[{id:fullSizeProducts()[0]?.id||"signature-relax",quantity:1}];const form=$("#manualOrderForm");if(form){form.reset();form.elements.customer_city.value="Полтава";form.elements.source.value="instagram";toggleManualDeliveryFields();}renderManualItems();const msg=$("#manualOrderMessage");if(msg)msg.textContent="";}
function renderManualItems(){const host=$("#manualItems");if(!host)return;host.innerHTML=manualItems.map((row,i)=>`<div class="admin2-manual-item" data-manual-row="${i}"><select data-manual-product>${manualProductOptions()}</select><input data-manual-qty type="number" min="1" max="10" value="${row.quantity}"><button type="button" data-manual-remove aria-label="Видалити">×</button></div>`).join("");host.querySelectorAll("[data-manual-row]").forEach(el=>{const i=Number(el.dataset.manualRow),select=el.querySelector("[data-manual-product]"),qty=el.querySelector("[data-manual-qty]");select.value=manualItems[i].id;select.onchange=()=>{manualItems[i].id=select.value;renderManualTotal();};qty.oninput=()=>{manualItems[i].quantity=Math.max(1,Math.min(10,Number(qty.value||1)));renderManualTotal();};el.querySelector("[data-manual-remove]").onclick=()=>{if(manualItems.length<=1)return;manualItems.splice(i,1);renderManualItems();};});renderManualTotal();}
function renderManualTotal(){const total=manualItems.reduce((s,row)=>s+productPrice(productById(row.id))*Number(row.quantity||1),0);const el=$("#manualOrderTotal");if(el)el.textContent=money(total);}
...
const body={
  ...
  items:manualItems.map(row=>({id:row.id,quantity:Number(row.quantity||1),selections:[]}))
};
...
if(result.order?.id){await sb.from("orders").update({source:String(fd.get("source")||"admin")}).eq("id",result.order.id).catch(()=>{});
}msg.textContent=result.payment_url?"Замовлення створено. Платіжне посилання скопійовано.":"Замовлення створено.";
...
function bind(){
  ...
  const globalSearch=$("#adminGlobalSearch");if(globalSearch){globalSearch.addEventListener("input",renderGlobalSearch);globalSearch.addEventListener("keydown",event=>{if(event.key==="Escape"){globalSearch.value="";renderGlobalSearch();globalSearch.blur();}});}
  ...
  document.addEventListener("click",event=>{const box=$("#adminSearchResults");if(box&&!event.target.closest(".admin2-global-search"))box.hidden=true;});
  ...
  $("#newOrderBtn")?.addEventListener("click",()=>{resetManualOrder();$("#manualOrderDialog").showModal();});
  ...
  $("#manualOrderForm")?.elements.delivery_method?.addEventListener("change",toggleManualDeliveryFields);
  $("#manualAddItem")?.addEventListener("click",()=>{manualItems.push({id:fullSizeProducts()[0]?.id||"signature-relax",quantity:1});renderManualItems();});$("#manualOrderForm")?.addEventListener("submit",createManualOrder);
