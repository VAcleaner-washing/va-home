...
if (content.release !== "15.2.0") fail(`central release is ${content.release}, expected 15.2.0`);
...
if (!productsJs.includes('const PRODUCT_CONTENT_VERSION = "15.2.0";')) fail("products.js content version mismatch");
...
for (const product of products) {
  ...
  if (!html.includes('id="reedSetupSection"') || !html.includes('id="productUsageSection"') || !html.includes("css/reed-guide.css?v=15.2.0") || !html.includes("css/fragrance-dna.css?v=15.2.0")) fail(`${product.id}: customer-friendly reed guide, six-axis DNA stylesheet, usage section or stylesheet missing`);
...
for (const file of walk(root).filter((file) => file.endsWith(".html"))) {
  ...
  if (/\?v=(?!15\.2\.0\b)\d+\.\d+\.\d+/.test(html)) fail(`stale asset version: ${path.relative(root, file)}`);
}
if (!read("service-worker.js").includes("const VERSION = '15.2.0';")) fail("root service worker version mismatch");
if (!read("admin/service-worker.js").includes("1.0.0-15.2.0")) fail("admin service worker version mismatch");
