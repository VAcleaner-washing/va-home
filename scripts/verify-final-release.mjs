...
for (const file of htmlFiles) {
  ...
  if (/\?v=(?!15\.2\.0\b)\d+\.\d+\.\d+/.test(html)) errors.push(`stale asset version: ${path.relative(root, file)}`);
...
if (!serviceWorkerJs.includes("production-9")) errors.push("public PWA cache revision is stale after card payment release");

const release = JSON.parse(fs.readFileSync(path.join(root, "release.json"), "utf8"));
if (release.version !== "15.2.0") errors.push(`release.json version is ${release.version}`);
