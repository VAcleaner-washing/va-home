...
for(const f of html){const s=fs.readFileSync(f,'utf8');
 ...
 if(/\?v=13\.8\.(?!16\b)\d+/.test(s)) errors.push(`stale asset version: ${f}`);
...
console.log(JSON.stringify({ok:true,html_pages:html.length,product_pages:html.filter(f=>f.includes(`${path.sep}products${path.sep}`)).length,version:'13.8.16'},null,2));
